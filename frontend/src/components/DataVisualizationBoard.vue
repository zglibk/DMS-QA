...
const initCharts = async () => {
  ...
  if (metricsEl) {
    ...
    charts.metricsTrend.setOption({
      ...
      series: [
        { name: '交检批次', type: 'bar', barWidth: UNIFIED_BAR_WIDTH + 6, barMaxWidth: UNIFIED_BAR_WIDTH + 6, barCategoryGap: '56%', z: 2, itemStyle: { color: seriesPalette.metricBatchInspect, borderRadius: [9, 9, 0, 0] }, data: finalTrendRows.map(item => Number(item.InspectionBatches) || 0) },
        { name: '发货批次', type: 'bar', barWidth: Math.max(UNIFIED_BAR_WIDTH - 4, 10), barMaxWidth: Math.max(UNIFIED_BAR_WIDTH - 4, 10), barCategoryGap: '56%', barGap: '-100%', z: 3, itemStyle: { color: seriesPalette.metricBatchDelivery, borderColor: seriesPalette.metricBatchDelivery, borderWidth: 0, borderRadius: [7, 7, 0, 0], opacity: 0.95 }, data: finalTrendRows.map(item => Number(item.DeliveryBatches) || 0) },
