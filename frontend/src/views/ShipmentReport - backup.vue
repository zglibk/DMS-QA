      ...
      <div class="shipment-report-page">
        ...
        <div class="page-header">
          ...
          <div class="header-actions">
            <el-button type="primary" plain @click="showTemplateManager = true">
            ...
            <el-card v-if="result.productInfo && result.productInfo.length > 0" shadow="hover" class="result-card product-card">
              <template #header>
                <div class="card-header">
                  ...
                  <el-button v-if="selectedProducts.length > 1" type="success" size="small" style="margin-left: auto" @click="handleBatchGenerateReport">
              ...
              >
                ...
                <el-table-column label="操作" width="120" align="center" fixed="right">
                  <template #default="{ row }">
                    <el-button type="primary" size="small" @click.stop="handleGenerateReport(row)">
              ...
              <template #header>
                <div class="card-header">
                  ...
                  <div class="preview-actions">
                    <el-button type="success" size="small" @click="handleExportReport"><el-icon><Download /></el-icon>导出</el-button>
                    <el-button type="primary" size="small" @click="handlePrintReport"><el-icon><Printer /></el-icon>打印</el-button>
                  </div>
                </div>
              </template>
          ...
          <el-table :data="templateList" stripe border>
            ...
            <el-table-column label="操作" width="180" align="center">
              <template #default="{ row }">
                ...
                <el-button type="success" link size="small" @click="openMappingDialog(row)">映射</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-dialog>
...
async function exportFromTemplate(template) {
  try {
    ...
    // 保存所有工作表的原始列宽，防止写入数据后列宽变化
    const originalColumnWidths = []
    workbook.worksheets.forEach((ws, wsIndex) => {
      originalColumnWidths[wsIndex] = {}
      ws.columns.forEach((col, colIndex) => {
        if (col.width) {
          originalColumnWidths[wsIndex][colIndex] = col.width
        }
      })
    })
    ...
    // 恢复所有工作表的原始列宽
    workbook.worksheets.forEach((ws, wsIndex) => {
      if (originalColumnWidths[wsIndex]) {
        Object.keys(originalColumnWidths[wsIndex]).forEach(colIndex => {
          const col = ws.getColumn(parseInt(colIndex) + 1) // ExcelJS列索引从1开始
          if (col) {
            col.width = originalColumnWidths[wsIndex][colIndex]
          }
        })
      }
    })
