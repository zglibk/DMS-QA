...
async function loadPersonList() {
  try {
    const response = await axios.get('/api/person/list?pageSize=1000&includeInactive=false')
...
async function loadTableData() {
  ...
  try {
    ...
    const response = await axios.get('/api/sample/list', { params })
...
async function loadStatistics() {
  try {
    const response = await axios.get('/api/sample/statistics')
...
async function handleAdd() {
  ...
  try {
    // 获取下一个样版编号预览
    const response = await axios.get('/api/sample/next-certificate-no')
    if (response.data.code === 200) {
...
async function handleDelete(row) {
  try {
    ...
    const response = await axios.delete(`/api/sample/delete/${row.id}`)
...
async function handleBatchDelete() {
  ...
  try {
    ...
    const response = await axios.delete('/api/sample/batch-delete', { data: { ids } })
...
async function handleSubmit() {
  try {
    ...
    if (submitData.id) {
      // 更新
      response = await axios.put(`/api/sample/update/${submitData.id}`, submitData)
    } else {
      // 新增 - 移除id和certificateNo字段，由后端自动生成样版编号
      delete submitData.id
      delete submitData.certificateNo
      response = await axios.post('/api/sample/create', submitData)
    }
