    ...
    <el-container>
      ...
      <el-header class="admin-header">
        ...
        <div class="header-right">
          ...
          <!-- 校准预警按钮 -->
          <el-tooltip content="校准预警" placement="bottom">
            <span class="warning-icon-wrap" @click="showWarningDialog = true">
              <el-badge :value="warningCount" :hidden="warningCount === 0" :max="99">
                <el-icon class="warning-icon" :class="{ 'has-warning': warningCount > 0 }">
                  <WarningFilled />
                </el-icon>
              </el-badge>
            </span>
...
import { HomeFilled, OfficeBuilding, User, Fold, Expand, Grid, Document, Lock, BellFilled, FullScreen, Aim, Setting, Tools, Upload, FolderOpened, CopyDocument, List, Money, Avatar, Briefcase, UserFilled, Menu, ArrowDown, ArrowUp, WarningFilled } from '@element-plus/icons-vue'
...
import CalibrationWarningDialog from '../../components/CalibrationWarningDialog.vue'
...
async function fetchWarningCount() {
  try {
    ...
    if (response.data && response.data.data) {
      warningCount.value = response.data.data.summary?.total || 0
    }
  } catch (error) {
...
// 检查并自动生成预警通知（每日一次）
async function autoGenerateWarningNotice() {
  try {
    // 检查今日是否已自动生成过
    const lastAutoGenerate = localStorage.getItem('calibrationWarningAutoGenerated')
    const today = new Date().toDateString()
    if (lastAutoGenerate === today) {
      return // 今日已自动生成过，跳过
    }
    ...
    if (response.data && response.data.code === 200) {
      if (response.data.data.created > 0) {
        console.log(`[自动预警] 成功生成预警通知，包含${response.data.data.instrumentCount}项预警`)
        // 刷新通知铃铛
        await userStore.fetchUnreadNoticeCount()
      }
      // 记录今日已自动生成
      localStorage.setItem('calibrationWarningAutoGenerated', today)
    }
  } catch (error) {
    console.error('[自动预警] 生成预警通知失败:', error)
  }
}
...
async function checkAndShowWarning() {
  ...
  // 自动生成预警通知（每日一次）
  await autoGenerateWarningNotice()
  ...
  // 如果有预警且有紧急预警（7天内到期或已过期），显示弹窗
  if (warningCount.value > 0) {
    try {
      const response = await instrumentApi.getCalibrationWarnings({ warningDays: 7 })
      if (response.data && response.data.data) {
        const urgentCount = (response.data.data.summary?.expired || 0) + (response.data.data.summary?.critical || 0)
        if (urgentCount > 0) {
          showWarningDialog.value = true
        }
      }
    } catch (error) {
      console.error('检查紧急预警失败:', error)
    }
  }
}
...
.warning-icon-wrap :deep(.el-badge__content) {
