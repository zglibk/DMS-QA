...
PRINT '  - publishing_exceptions: 出版失误管理表'
PRINT '包含触发器：'
...
PRINT '  - publishing_exceptions表相关索引: 登记日期、客户代码、工单号、删除状态、错误类型'
PRINT '包含测试数据：'
...
-- =====================================================
-- 考核记录管理系统相关表结构
-- 功能：质量考核记录管理、改善期跟踪、考核返还处理
-- 基于：ComplaintRegister、ProductionReworkRegister、publishing_exceptions
-- =====================================================

-- 检查并创建考核记录表 (AssessmentRecords)
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'AssessmentRecords')
BEGIN
    CREATE TABLE [dbo].[AssessmentRecords] (
        ...
        [SourceType] NVARCHAR(20) NOT NULL DEFAULT 'complaint', -- 来源类型：complaint/rework/exception
        ...
        -- 部门关联字段
        [DepartmentID] INT NULL,                               -- 部门ID，关联Department表
        [DepartmentName] NVARCHAR(50) NULL,                    -- 部门名称（冗余字段，便于查询显示）
        
        -- 计算字段：考核月份（格式：202501）
        [AssessmentMonth] AS (YEAR(AssessmentDate) * 100 + MONTH(AssessmentDate)),
        
        -- 状态管理字段
        [Status] NVARCHAR(20) DEFAULT 'pending',               -- 状态：pending/improving/returned/confirmed/exempt
        ...
        -- 外键约束
        CONSTRAINT FK_AssessmentRecords_Department
            FOREIGN KEY (DepartmentID) REFERENCES [dbo].[Department](ID)
    );
    ...
    CREATE INDEX IX_AssessmentRecords_SourceType ON AssessmentRecords (SourceType);
    CREATE INDEX IX_AssessmentRecords_DepartmentID ON AssessmentRecords (DepartmentID);
    
    PRINT '✅ 考核记录表 (AssessmentRecords) 创建成功';
END
ELSE
BEGIN
    PRINT '⚠️ 考核记录表 (AssessmentRecords) 已存在，跳过创建';
END

-- 检查并创建改善期跟踪表 (ImprovementTracking)
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'ImprovementTracking')
BEGIN
    ...
    PRINT '✅ 改善期跟踪表 (ImprovementTracking) 创建成功';
END
ELSE
BEGIN
    PRINT '⚠️ 改善期跟踪表 (ImprovementTracking) 已存在，跳过创建';
END

-- 检查并创建考核配置表 (AssessmentConfig)
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'AssessmentConfig')
BEGIN
    ...
    PRINT '✅ 考核配置表 (AssessmentConfig) 创建成功';
END
ELSE
BEGIN
    PRINT '⚠️ 考核配置表 (AssessmentConfig) 已存在，跳过创建';
END

-- 检查并创建考核历史表 (AssessmentHistory)
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'AssessmentHistory')
BEGIN
    ...
    PRINT '✅ 考核历史表 (AssessmentHistory) 创建成功';
END
ELSE
BEGIN
    PRINT '⚠️ 考核历史表 (AssessmentHistory) 已存在，跳过创建';
END

-- 创建考核统计视图
IF NOT EXISTS (SELECT * FROM sys.views WHERE name = 'V_AssessmentStatistics')
BEGIN
    EXEC('
    ...
    SELECT 
        PersonName,                                            -- 人员姓名
        PersonType,                                            -- 人员类型
        YEAR(AssessmentDate) as AssessmentYear,                -- 考核年份
        MONTH(AssessmentDate) as AssessmentMonth,              -- 考核月份
        COUNT(*) as AssessmentCount,                           -- 考核次数
        SUM(AssessmentAmount) as TotalAssessmentAmount,        -- 总考核金额
        SUM(CASE WHEN IsReturned = 1 THEN ReturnAmount ELSE 0 END) as TotalReturnAmount, -- 总返还金额
        COUNT(CASE WHEN IsReturned = 1 THEN 1 END) as ReturnCount -- 返还次数
    FROM AssessmentRecords
    WHERE Status IN (''pending'', ''improving'', ''returned'', ''confirmed'')
    GROUP BY PersonName, PersonType, YEAR(AssessmentDate), MONTH(AssessmentDate)
    ');
    
    PRINT '✅ 考核统计视图 (V_AssessmentStatistics) 创建成功';
END
ELSE
BEGIN
...
BEGIN
    ...
    INSERT INTO AssessmentRecords (
        ...
        DepartmentID, DepartmentName, SourceType, Status, CreatedBy, CreatedAt
    )
    SELECT 
        cr.ID,
        cr.MainPerson,
    ...
    LEFT JOIN Person p ON cr.MainPerson = p.Name
    ...
    WHERE cr.MainPerson IS NOT NULL 
      ...
      AND NOT EXISTS (
          ...
          WHERE ar.ComplaintID = cr.ID AND ar.PersonType = 'MainPerson' AND ar.SourceType = 'complaint'
      );
    ...
    SELECT 
        ...
        GETDATE()
    FROM ComplaintRegister cr
    LEFT JOIN Person p ON cr.SecondPerson = p.Name
    LEFT JOIN Department d ON p.DepartmentID = d.ID
    WHERE cr.SecondPerson IS NOT NULL 
      ...
      AND NOT EXISTS (
          SELECT 1 FROM AssessmentRecords ar 
          WHERE ar.ComplaintID = cr.ID AND ar.PersonType = 'SecondPerson' AND ar.SourceType = 'complaint'
      );
    ...
    SELECT 
        ...
        GETDATE()
    FROM ComplaintRegister cr
    LEFT JOIN Person p ON cr.Manager = p.Name
    LEFT JOIN Department d ON p.DepartmentID = d.ID
    WHERE cr.Manager IS NOT NULL 
      ...
      AND NOT EXISTS (
          SELECT 1 FROM AssessmentRecords ar 
          WHERE ar.ComplaintID = cr.ID AND ar.PersonType = 'Manager' AND ar.SourceType = 'complaint'
      );
    ...
    LEFT JOIN Person p ON prr.ResponsiblePerson = p.Name
    LEFT JOIN Department d ON p.DepartmentID = d.ID
    WHERE prr.ResponsiblePerson IS NOT NULL 
      ...
      AND NOT EXISTS (
          SELECT 1 FROM AssessmentRecords ar 
          WHERE ar.ComplaintID = prr.ID AND ar.PersonType = 'MainPerson' AND ar.SourceType = 'rework'
      );
    ...
    SELECT 
        ...
        'MainPerson',
        ISNULL(pe.unit_price * pe.area_cm2 / 10000, 0),
    ...
    LEFT JOIN Person p ON pe.responsible_person = p.Name
    LEFT JOIN Department d ON p.DepartmentID = d.ID
    WHERE pe.responsible_person IS NOT NULL 
      ...
      AND NOT EXISTS (
          SELECT 1 FROM AssessmentRecords ar 
          WHERE ar.ComplaintID = pe.id AND ar.PersonType = 'MainPerson' AND ar.SourceType = 'exception'
      );
...
PRINT '✅ 生成考核记录存储过程 (SP_GenerateAssessmentRecords) 更新完成';
...
BEGIN
    ...
    BEGIN
        ...
        WITH EligibleReturns AS (
            ...
            WHERE ar.Status IN (''pending'', ''improving'')
                AND ar.IsReturned = 0
                ...
                AND NOT EXISTS (
                    ...
                    WHERE ar2.PersonName = ar.PersonName
                        ...
                        AND ar2.Status IN (''pending'', ''improving'')
    ...
    PRINT '✅ 改善期返还处理存储过程 (SP_ProcessImprovementReturns) 创建成功';
END
ELSE
BEGIN
    PRINT '⚠️ 改善期返还处理存储过程 (SP_ProcessImprovementReturns) 已存在，跳过创建';
END

PRINT '✅ 考核记录管理系统相关表结构创建完成';
...
BEGIN
    CREATE TABLE [dbo].[Instruments] (
        ...
        [UpdatedBy] INT,                                   -- 更新人
        [UpdatedAt] DATETIME DEFAULT GETDATE(),            -- 更新时间
        [MeasurementRange] NVARCHAR(200),                  -- 测量范围
        [Accuracy] NVARCHAR(100),                          -- 精度等级
...
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_CalibrationResults_InstrumentCode')
    CREATE NONCLUSTERED INDEX [IX_CalibrationResults_InstrumentCode] ON [dbo].[CalibrationResults] ([InstrumentCode]);

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_CalibrationResults_ManagementCode')
    CREATE NONCLUSTERED INDEX [IX_CalibrationResults_ManagementCode] ON [dbo].[CalibrationResults] ([ManagementCode]);
...
-- 创建触发器：当Instruments表的InstrumentCode或ManagementCode更新时，同步更新CalibrationResults表
IF NOT EXISTS (SELECT * FROM sys.triggers WHERE name = 'TR_CalibrationResults_SyncInstrumentCode')
BEGIN
    EXEC('
    CREATE TRIGGER [dbo].[TR_CalibrationResults_SyncInstrumentCode]
    ON [dbo].[CalibrationResults]
    AFTER INSERT, UPDATE
    AS
    BEGIN
        SET NOCOUNT ON;
        
        -- 当插入或更新CalibrationResults时，自动同步InstrumentCode和ManagementCode
        UPDATE cr
        SET InstrumentCode = i.InstrumentCode,
            ManagementCode = i.ManagementCode
        FROM [dbo].[CalibrationResults] cr
        INNER JOIN inserted ins ON cr.ID = ins.ID
        INNER JOIN [dbo].[Instruments] i ON cr.InstrumentID = i.ID
        WHERE cr.InstrumentCode IS NULL OR cr.InstrumentCode != i.InstrumentCode
           OR cr.ManagementCode IS NULL OR cr.ManagementCode != i.ManagementCode;
    END
    ');
    PRINT '✅ CalibrationResults 同步触发器创建成功';
END
ELSE
BEGIN
    PRINT '⚠️ CalibrationResults 同步触发器已存在，跳过创建';
END
...
PRINT '- CalibrationResults表包含InstrumentCode和ManagementCode冗余字段';
PRINT '- 移除了维护记录功能模块';
PRINT '- 重新设计为第三方校准结果登记';
PRINT '- 支持年度检定计划和实施表导出';
PRINT '- 包含数据一致性触发器，自动同步仪器编号和管理编号';
PRINT '';
