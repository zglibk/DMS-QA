...
router.post('/:id/parse', authenticateToken, async (req, res) => {
  try {
    ...
    // 调试：打印第一个有内容的单元格的样式信息
    let debugCell = null
    worksheet.eachRow((row, rowNumber) => {
      if (!debugCell && rowNumber <= 5) {
        row.eachCell((cell, colNumber) => {
          if (!debugCell && cell.value) {
            debugCell = {
    ...
    if (debugCell) {
      console.log('Sample cell style:', JSON.stringify(debugCell, null, 2))
    }
...
router.post('/:id/generate', authenticateToken, async (req, res) => {
  try {
    ...
    const { data: recordsData } = req.body
